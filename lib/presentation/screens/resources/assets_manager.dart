const String IMAGE_PATH = 'assets/images';
const String LOTTIE_PATH = 'assets/lottie';

class ImageAssets {
  // static const String splashLogo = '$IMAGE_PATH/logo.png';
  // static const String splashLogo = '$IMAGE_PATH/splash_logo.png';

  static const String splashLogo = '$IMAGE_PATH/percel_point_logo.png';


  static const String onBoardingLogo1 = '$IMAGE_PATH/onboarding_logo1.svg';
  static const String onBoardingLogo2 = '$IMAGE_PATH/onboarding_logo2.svg';
  static const String onBoardingLogo3 = '$IMAGE_PATH/onboarding_logo3.svg';
  static const String onBoardingLogo4 = '$IMAGE_PATH/onboarding_logo4.svg';

  static const String hollowCircle =
      '$IMAGE_PATH/hollow_cirlce_ic.svghollow_cirlce_ic.svg';
  static const String solidCircle = '$IMAGE_PATH/solid_circle_ic.svg';
  static const String leftArrow = '$IMAGE_PATH/left_arrow_ic.svg';
  static const String rightArrow = '$IMAGE_PATH/right_arrow_ic.svg';

  static const String list = '$IMAGE_PATH/list.png';

  static const String inactiveAccount = '$LOTTIE_PATH/inactive.json';
  static const String loader = '$LOTTIE_PATH/no_data.json';
  static const String noInternet = '$LOTTIE_PATH/no_internet.json';
  static const String noInternetWomen = '$LOTTIE_PATH/no_internet_women.json';
  static const String noData = '$LOTTIE_PATH/no_data.json';
}
